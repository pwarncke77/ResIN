...
A list object containing the original dataframe, (`resin_df`), the
dummy-coded dataframe (`resin_dummies`), the ResIN correlation and
covariance matrices (`resin_cor` & `resin_vcov`), and a numeric vector
detailing which item responses belong to which item (`same_items`).
